...
async function renderRoomMessages(roomName) {
    ...
    if (!msgs || msgs.length === 0) { container.innerHTML = '<div class="empty-state">💬 შეტყობინებები არ არის</div>'; return; }
...
async function renderPMMessages(chatId) {
    ...
    if (!msgs || msgs.length === 0) { container.innerHTML = '<div style="text-align:center;padding:15px;opacity:0.5;font-size:12px;">მიმოწერა ცარიელია</div>'; return; }
...
async function sendPM() {
    ...
    const { error } = await db.from('direct_messages').insert([{
        ...
        text: text || "", image: pendingPMImage || null, time: Date.now().toString()
    }]);
    if (error) { showNotification('PM შეცდომა: ' + error.message); return; }
    input.value = '';
    clearPMPendingImg();
...
async function renderPMList() {
    ...
    let seen = new Set();
    chats?.forEach(c => seen.add(c.from_user === currentUser ? c.to_user : c.from_user));
    ...
    document.getElementById('pmContainer').innerHTML = `
        <div class="pm-window">
            <div class="pm-header">
                ...
                <span onclick="closePM()" style="cursor:pointer;font-size:16px;">✕</span>
            </div>
            <div class="pm-messages" id="pmMessages"></div>
            <div id="pmPendingWrap" style="display:none;padding:3px 8px;align-items:center;gap:5px;flex-direction:row;">
            ...
            <div id="pmEmojiPanel" style="display:none;padding:8px;background:var(--card);border-top:1px solid var(--accent);max-height:160px;overflow-y:auto;">
                <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;">
                    ...
                    ${customEmojis.map(e => typeof e === 'string' && e.startsWith('data:')
                        ? `<img src="${e}" style="width:28px;height:28px;object-fit:cover;border-radius:5px;cursor:pointer;" onclick="insertPMCustomEmoji('${e}')">`
                        : `<div style="font-size:22px;text-align:center;cursor:pointer;padding:4px;" onclick="insertPMEmoji('${e}')">${e}</div>`
                    ).join('')}
                </div>
            </div>
            <div class="pm-input">
...
function addPMPhoto(input) {
    ...
    reader.onload = e => {
        ...
        let preview = document.getElementById('pmPendingPreview');
        let wrap = document.getElementById('pmPendingWrap');
        if (preview) preview.src = pendingPMImage;
        if (wrap) wrap.style.display = 'flex';
    };
    reader.readAsDataURL(file);
    input.value = '';
...
async function listenToFriendRequests() {
    // realtime მეგობრობის მოთხოვნებზე
    db.channel('friends_' + currentUser)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'friend_requests', filter: `to_user=eq.${currentUser}` },
            (payload) => {
                showNotification(`📨 ${payload.new.from_user}-მა მეგობრობა მოითხოვა!`);
                renderRequestsList();
            })
...
async function isFriend(username) {
    const { data } = await db.from('friend_requests').select('id')
        .eq('status', 'accepted')
...
async function sendFriendRequest(target) {
    ...
    if (existing && existing.length > 0) {
        if (existing[0].status === 'accepted') return showNotification('უკვე მეგობარია');
...
async function acceptFriendRequest(from) {
    const { error } = await db.from('friend_requests').update({ status: 'accepted' })
        .eq('from_user', from).eq('to_user', currentUser).eq('status', 'pending');
    if (!error) { showNotification(`✅ ${from} დაემატა!`); renderAll(); }
}

async function rejectFriendRequest(from) {
    await db.from('friend_requests').delete().eq('from_user', from).eq('to_user', currentUser).eq('status', 'pending');
...
async function renderFriendsList() {
    ...
    if (!container) return;
    const { data } = await db.from('friend_requests').select('from_user,to_user').eq('status', 'accepted')
        .or(`from_user.eq.${currentUser},to_user.eq.${currentUser}`);
    let friendNames = (data || []).map(r => r.from_user === currentUser ? r.to_user : r.from_user);
    let friendUsers = users.filter(u => friendNames.includes(u.username));
...
async function renderRequestsList() {
    ...
    container.innerHTML = !pending || pending.length === 0
...
async function renderUsersList() {
    ...
    if (!container) return;
    const { data: allReqs } = await db.from('friend_requests').select('*')
        .or(`from_user.eq.${currentUser},to_user.eq.${currentUser}`);
    ...
    container.innerHTML = others.map(user => {
        let req = (allReqs || []).find(r =>
            (r.from_user === currentUser && r.to_user === user.username) ||
            (r.from_user === user.username && r.to_user === currentUser));
        let isFr = req?.status === 'accepted';
        let pendFromMe = req?.status === 'pending' && req?.from_user === currentUser;
        let pendToMe = req?.status === 'pending' && req?.to_user === currentUser;
...
async function renderAll() { renderUsersList(); renderFriendsList(); renderRequestsList(); }
...
function showQuizQuestion() {
    let existing = document.getElementById('quizBox');
    ...
    box.innerHTML = `
        ...
        <div id="quizOptions" style="display:flex;flex-direction:column;gap:7px;">
            ${(options || []).map((opt, i) =>
                `<button onclick="answerQuiz(${i})" style="background:#2a1a1a;border:1px solid var(--accent);color:var(--text);padding:8px 12px;border-radius:10px;cursor:pointer;text-align:left;font-size:13px;">${escapeHtml(String(opt))}</button>`
            ).join('')}
        </div>
...
function answerQuiz(selectedIndex) {
    ...
    let correctIndex = typeof q.answer === 'number' ? q.answer : (options || []).findIndex(o => o === q.answer);
    buttons.forEach((btn, i) => {
        btn.disabled = true;
        if (i === correctIndex) btn.style.background = '#2d6a4f';
        else if (i === selectedIndex && i !== correctIndex) btn.style.background = '#8b0000';
    });
    if (selectedIndex === correctIndex) { quizScore++; showNotification('✅ სწორია! +1 ქულა'); }
    else showNotification('❌ არასწორი!');
    quizIndex++;
...
async function saveStatus() {
    ...
    let me = users.find(u => u.username === currentUser);
    if (me) { me.status = status; saveAll(); }
...
function openPhotoModal(photoUrl, username) {
    ...
    modal.querySelector('#likeBtn').onclick = () => {
        ...
        saveAll(); modal.querySelector('#likeBtn').innerHTML = `❤️ ${data.likes.length}`;
    };
    if (modal.querySelector('#setProfileBtn')) modal.querySelector('#setProfileBtn').onclick = () => { setAsProfilePhoto(photoUrl); modal.remove(); };
...
async function showUserProfile(username) {
    ...
    let amAdmin = isAdminOrOwner(currentUser);
    ...
    if (modal.querySelector('#blockBtn')) modal.querySelector('#blockBtn').onclick = () => {
        if (!blockedUsers.includes(username)) { blockedUsers.push(username); saveAll(); showNotification(`🔒 ${username} დაბლოკილია`); }
        modal.remove();
    };
    if (modal.querySelector('#unblockBtn')) modal.querySelector('#unblockBtn').onclick = () => {
        blockedUsers = blockedUsers.filter(u => u !== username); saveAll(); showNotification(`🔓 ${username} განბლოკილია`);
        modal.remove();
    };
    if (modal.querySelector('#makeAdminBtn')) modal.querySelector('#makeAdminBtn').onclick = () => {
        let u = users.find(x => x.username === username);
        if (u) { u.role = 'admin'; u.status = '🔧 ადმინი'; saveAll(); showNotification(`✅ ${username} ადმინია!`); }
        modal.remove();
    };
    if (modal.querySelector('#removeAdminBtn')) modal.querySelector('#removeAdminBtn').onclick = () => {
        let u = users.find(x => x.username === username);
        if (u) { u.role = 'user'; u.status = 'ონლაინ'; saveAll(); showNotification(`🔻 ადმინი ჩამოერთვა`); }
        modal.remove();
    };
    if (modal.querySelector('#makeOwnerBtn')) modal.querySelector('#makeOwnerBtn').onclick = () => {
        let old = users.find(u => u.role === 'owner');
        if (old) old.role = 'user';
        let u = users.find(x => x.username === username);
        if (u) { u.role = 'owner'; u.status = '👑 დამფუძნებელი'; saveAll(); showNotification(`👑 ${username} დამფუძნებელია!`); }
        modal.remove();
    };
    modal.onclick = e => { if (e.target === modal) modal.remove(); };
    document.body.appendChild(modal);
...
async function init() {
    ...
    document.addEventListener('click', e => {
        let panel = document.getElementById('emojiPanel');
        if (panel && !panel.contains(e.target) && !e.target.classList.contains('icon-btn')) panel.style.display = 'none';
    });
