...
async function renderPMMessages(chatId) {
    ...
    const { data: msgs, error } = await db
        ...
        .eq('chat_id', chatId)
        .order('time', { ascending: true });
    if (error) { console.error('PM error:', error); return; }
    if (!msgs || msgs.length === 0) {
        container.innerHTML = '<div style="text-align:center;padding:15px;opacity:0.5;font-size:12px;">მიმოწერა ცარიელია</div>';
        return;
    }
...
function showQuizQuestion() {
    ...
    // კითხვა შეიძლება იყოს: question, options (array ან JSON string), answer
    ...
    box.innerHTML = `
        ...
        <div id="quizOptions" style="display:flex;flex-direction:column;gap:7px;">
            ${options.map((opt, i) => `
                <button onclick="answerQuiz(${i})" style="background:#2a1a1a;border:1px solid var(--accent);color:var(--text);padding:8px 12px;border-radius:10px;cursor:pointer;text-align:left;font-size:13px;">${escapeHtml(opt)}</button>
            `).join('')}
        </div>
...
function answerQuiz(selectedIndex) {
    ...
    // answer შეიძლება იყოს index (number) ან ტექსტი
    if (typeof q.answer === 'number') {
        correctIndex = q.answer;
    } else {
        correctIndex = options.findIndex(o => o === q.answer);
    }

    buttons.forEach((btn, i) => {
        btn.disabled = true;
        if (i === correctIndex) btn.style.background = '#2d6a4f';
        else if (i === selectedIndex) btn.style.background = '#8b0000';
    });
...
function openPhotoModal(photoUrl, username) {
    ...
    modal.innerHTML = `<div class="photo-modal-content">
        ...
        <div class="photo-comments" id="photoComments">
            ${data.comments.length
                ? data.comments.map(c => `<div class="comment-item"><b>${escapeHtml(c.user)}</b>: ${escapeHtml(c.text)}</div>`).join('')
    ...
    modal.querySelector('#addCommentBtn').onclick = () => {
        ...
        saveAll();
        modal.querySelector('#photoComments').innerHTML = data.comments.map(c =>
            `<div class="comment-item"><b>${escapeHtml(c.user)}</b>: ${escapeHtml(c.text)}</div>`).join('');
        input.value = '';
    };
