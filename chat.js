...
async function initData() {
    ...
    if (profilesData) {
        profilesData.forEach(p => {
            ...
            if (!ex) {
                users.push({ username: p.username, status: p.status || "ონლაინ", avatar: p.avatar || null, role: p.username === OWNER ? 'owner' : (p.role === 'Admin' ? 'admin' : 'user'), online: false });
            } else {
...
async function renderRoomMessages(roomName) {
    ...
    container.innerHTML = (!msgs || msgs.length === 0) ? '<div class="empty-state">💬 შეტყობინებები არ არის</div>' : msgs.map(buildMessageHtml).join('');
...
function buildMessageHtml(msg) {
    ...
    return `<div class="msg-row ${isOwn ? 'own' : ''}" id="msgrow-${msg.id}">
        ...
        <div class="message" id="msg-${msg.id}">
            ...
            ${msg.image ? `<img src="${msg.image}" class="message-img" onclick="openPhotoModal('${encodeURIComponent(msg.image)}','${escapeHtml(msg.sender)}')">` : ''}
...
async function sendMessage() {
    ...
    const { error } = await db.from('messages').insert([{ sender: currentUser, text: text || "", room: currentRoom, image: pendingImage || null, time: Date.now().toString() }]);
...
async function renderPMMessages(chatId) {
    ...
    c.innerHTML = (!msgs || msgs.length === 0) ? '<div style="text-align:center;padding:15px;opacity:0.5;font-size:12px;">მიმოწერა ცარიელია</div>' : msgs.map(buildPMHtml).join('');
    c.scrollTop = c.scrollHeight;
}
...
async function sendPM() {
    ...
    const { error } = await db.from('direct_messages').insert([{ chat_id: chatId, from_user: currentUser, to_user: currentPMUser, text: text || "", image: pendingPMImage || null, time: Date.now().toString() }]);
    if (error) { showNotification('PM შეცდომა: ' + error.message); return; }
...
async function buildPMEmojiGrid() {
    const { data } = await db.from('custom_emojis').select('*').order('created_at', { ascending: true });
    let customs = data || [];
    return `<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;">
        ...
        ${customs.map(e => `<img src="${e.data}" style="width:28px;height:28px;object-fit:cover;border-radius:5px;cursor:pointer;margin:2px;" onclick="insertPMCustomEmoji('${e.data}')">`).join('')}
    </div>`;
}
...
function renderPrivateChat() {
    document.getElementById('pmContainer').innerHTML = `
        <div class="pm-window">
            ...
            <div id="pmPendingWrap" style="display:none;padding:3px 8px;align-items:center;gap:5px;">
...
function insertPMEmoji(emoji) {
    ...
    let panel = document.getElementById('pmEmojiPanel'); if (panel) panel.style.display = 'none';
}
...
async function toggleEmojiPanel() {
    ...
    panel.innerHTML = `<div class="emoji-grid">
        ...
        ${customs.map(e=>`<img src="${e.data}" style="width:32px;height:32px;object-fit:cover;border-radius:6px;cursor:pointer;padding:2px;" onclick="insertCustomEmoji('${e.data}')">`).join('')}
...
async function uploadEmoji(input) {
    ...
    reader.onload = async e => {
        const { error } = await db.from('custom_emojis').insert([{ uploaded_by: currentUser, data: e.target.result }]);
...
async function renderAlbum() {
    ...
    c.innerHTML = (!data||data.length===0)
        ...
        : data.map(p=>`<img src="${p.data}" class="album-photo" onclick="openPhotoModal('${p.id}','${escapeHtml(currentUser)}','${encodeURIComponent(p.data)}')">`).join('');
}
...
async function addToAlbum(input) {
    ...
    let reader = new FileReader();
    reader.onload = async e => {
        const { error } = await db.from('user_photos').insert([{ username: currentUser, data: e.target.result }]);
...
async function setAsProfilePhoto(photoData) {
    let me = users.find(u=>u.username===currentUser);
...
// ========== ფოტო მოდალი - SUPABASE ==========
async function openPhotoModal(photoId, username, photoDataEncoded) {
    let photoData = decodeURIComponent(photoDataEncoded || photoId);
    // ლაიქები
    const { data: likes } = await db.from('photo_likes').select('*').eq('photo_id', String(photoId));
    const { data: comments } = await db.from('photo_comments').select('*').eq('photo_id', String(photoId)).order('created_at', { ascending: true });
    let likeCount = (likes||[]).length;
    let hasLiked = (likes||[]).some(l=>l.from_user===currentUser);
    ...
    modal.innerHTML = `<div class="photo-modal-content">
        ...
        <div style="padding:9px;display:flex;gap:7px;flex-wrap:wrap;">
            <button class="action-btn" id="likeBtn" style="background:${hasLiked?'#5a0000':'var(--accent)'};">❤️ ${likeCount}</button>
        ...
        <div class="photo-comments" id="photoComments">
            ${(comments&&comments.length)?comments.map(c=>`<div class="comment-item"><b>${escapeHtml(c.from_user)}</b>: ${escapeHtml(c.text)}</div>`).join(''):'<div style="opacity:0.5;padding:6px;font-size:12px;">კომენტარები არ არის</div>'}
        </div>
    ...
    modal.querySelector('#likeBtn').onclick = async () => {
        if (hasLiked) {
            await db.from('photo_likes').delete().eq('photo_id', String(photoId)).eq('from_user', currentUser);
            hasLiked=false; likeCount--;
        } else {
            await db.from('photo_likes').insert([{ photo_id: String(photoId), from_user: currentUser }]);
            hasLiked=true; likeCount++;
        }
        modal.querySelector('#likeBtn').innerHTML = `❤️ ${likeCount}`;
        modal.querySelector('#likeBtn').style.background = hasLiked?'#5a0000':'var(--accent)';
    };
    ...
    modal.querySelector('#addCommentBtn').onclick = async () => {
        ...
        if (!error) {
            const { data: newComments } = await db.from('photo_comments').select('*').eq('photo_id', String(photoId)).order('created_at', { ascending: true });
            modal.querySelector('#photoComments').innerHTML = (newComments||[]).map(c=>`<div class="comment-item"><b>${escapeHtml(c.from_user)}</b>: ${escapeHtml(c.text)}</div>`).join('');
            input.value='';
...
async function showUserProfile(username) {
    ...
    // ალბომი
    const { data: photos } = await db.from('user_photos').select('*').eq('username', username).order('created_at', { ascending: true }).limit(6);
    ...
    modal.innerHTML = `<div class="modal-content">
        ...
        ${photos&&photos.length?`<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin:10px 0;">
            ${photos.map(p=>`<img src="${p.data}" style="aspect-ratio:1;object-fit:cover;border-radius:8px;cursor:pointer;border:1px solid var(--accent);" onclick="openPhotoModal('${p.id}','${escapeHtml(username)}','${encodeURIComponent(p.data)}')">`).join('')}
        </div>`:''}
...
async function init() {
    ...
    window.addEventListener('beforeunload',()=>{ let me=users.find(u=>u.username===currentUser); if(me){me.online=false;saveLocal();} });
}
